...
def toggle_menu():
    ...
    click=StringVar()
    click.set("Vehicle Hire")
    drop=OptionMenu(toggle_menu_fm,click,*options)
    drop.place(x=25,y=30)
    drop.config(width=10,background='#3AAFA9',foreground="lavender",font=("Helvetica 10 bold"),
                activebackground='#3AAFA9', activeforeground='white')
...
leaving = Label(frame_login, text='Leaving From', font=('times new roman', 25), bg='#3AAFA9', fg='white').place(x=40,
                                                                                                                y=40)
textbox1 = Entry(frame_login, font=('times new roman', 25)).place(x=40, y=90)
going = Label(frame_login, text='Going Destination', font=('times new roman', 25), bg='#3AAFA9', fg='white').place(x=40,
                                                                                                                   y=150)
textbox2 = Entry(frame_login, font=('times new roman', 25)).place(x=40, y=200)
Travel = Label(frame_login, text='Travel Date', font=('times new roman', 25), bg='#3AAFA9', fg='white').place(x=40,
                                                                                                              y=260)
textbox3= Entry(frame_login, font=('times new roman', 25)).place(x=40, y=310)
shift = Label(frame_login, text='Select Shift', font=('times new roman', 25), bg='#3AAFA9', fg='white').place(x=40,
                                                                                                              y=370)
var = IntVar()
...
def add():
    conn = sqlite3.connect('main.db')
    c = conn.cursor()
    c.execute("INSERT INTO main VALUES(:Full_name, :Number, :Address, :Email, :password, :retype_password)",{
        'leave':textbox1.get(),
        'going':textbox2.get(),
        'travel':textbox3.get(),
        'radio':var.get()
